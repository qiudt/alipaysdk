...
/**
 * AlipayFundTransCommonApi Class Doc Comment
 *
 * @category Class
 * @package  Alipay\OpenAPISDK
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 */
class AlipayFundTransCommonApi
{
    /**
     * @var ClientInterface
     */
     ...
     * 转账业务单据查询接口
     *
     * @param  string $productCode 销售产品码，商家和支付宝签约的产品码，如果传递了out_biz_no则该字段为必传。可传值如下： STD_RED_PACKET：现金红包 TRANS_ACCOUNT_NO_PWD：单笔无密转账到支付宝账户 TRANS_BANKCARD_NO_PWD：单笔无密转账到银行卡 (optional)
     * @param  string $bizScene 描述特定的业务场景，如果传递了out_biz_no则该字段为必传。可取的业务场景如下：  PERSONAL_PAY：C2C现金红包-发红包；  PERSONAL_COLLECTION：C2C现金红包-领红包；  REFUND：C2C现金红包-红包退回；  DIRECT_TRANSFER：B2C现金红包、单笔无密转账 (optional)
     * @param  string $outBizNo 商户转账唯一订单号，发起转账来源方定义的转账单据ID。  本参数和order_id（支付宝转账单据号）、pay_fund_order_id（支付宝支付资金流水号）三者不能同时为空。 当三者同时传入时，将用pay_fund_order_id（支付宝支付资金流水号）进行查询，忽略其余两者； 当本参数和支付宝转账单据号同时提供时，将用支付宝转账单据号进行查询，忽略本参数。 (optional)
     * @param  string $orderId 支付宝转账单据号。 本参数和out_biz_no（商户转账唯一订单号）、pay_fund_order_id（支付宝支付资金流水号）三者不能同时为空。  当三者同时传入时，将用pay_fund_order_id（支付宝支付资金流水号）进行查询，忽略其余两者； 当本参数和pay_fund_order_id（支付宝支付资金流水号）同时提供时，将用支付宝支付资金流水号进行查询，忽略本参数；  当本参数和out_biz_no（商户转账唯一订单号）同时提供时，将用本参数进行查询，忽略商户转账唯一订单号。 (optional)
     * @param  string $payFundOrderId 支付宝支付资金流水号。本参数和支付宝转账单据号、商户转账唯一订单号三者不能同时为空。  当本参数和out_biz_no（商户转账唯一订单号）、order_id（支付宝转账单据号）同时提供时，将用本参数进行查询，忽略其余两者； 当本参数和order_id（支付宝转账单据号）同时提供时，将用本参数进行查询，忽略支付宝转账单据号；  当本参数和out_biz_no（商户转账唯一订单号）同时提供时，将用本参数进行查询，忽略商户转账唯一订单号。 (optional)
     * @param CustomizedParams $customizedParams
     *
     * @throws \Alipay\OpenAPISDK\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryResponseModel|\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryDefaultResponse
     */
    public function query($productCode = null, $bizScene = null, $outBizNo = null, $orderId = null, $payFundOrderId = null, ?CustomizedParams $customizedParams = null)
    {
        list($response) = $this->queryWithHttpInfo($productCode, $bizScene, $outBizNo, $orderId, $payFundOrderId, $customizedParams);
        return $response;
    }
     ...
     * @throws \Alipay\OpenAPISDK\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryResponseModel|\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryDefaultResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function queryWithHttpInfo($productCode = null, $bizScene = null, $outBizNo = null, $orderId = null, $payFundOrderId = null, ?CustomizedParams $customizedParams = null)
    {
        $request = $this->queryRequest($productCode, $bizScene, $outBizNo, $orderId, $payFundOrderId, $customizedParams);
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryResponseModel' === '\SplFileObject') {
                        ...
                        if ('\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryResponseModel' !== 'string') {
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryResponseModel', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                                
                default:
                    if ('\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryDefaultResponse' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        //验签
                        $this->alipayConfigUtil->verifyResponse($content, $response->getHeaders(), true);
                        if ('\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryDefaultResponse' !== 'string') {
                            $content = json_decode($content);
                        }
                    }
                    AlipayLogger::logBizResponseInfo($response->getStatusCode(), $content, $response->getHeaders());
                    ...
                    return [
                        //ObjectSerializer::deserialize($content, '\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryDefaultResponse', []),
                        $content,
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            ...
            $returnType = '\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryResponseModel';
            ...
            switch ($e->getCode()) {
                ...
                default:
                    //$data = ObjectSerializer::deserialize(
                    //    $e->getResponseBody(),
                    //    '\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryDefaultResponse',
                    //    $e->getResponseHeaders()
                    //);
                    //$e->setResponseObject($data);
                    break;
     ...
     * @deprecated
     */
    public function queryAsync($productCode = null, $bizScene = null, $outBizNo = null, $orderId = null, $payFundOrderId = null, ?CustomizedParams $customizedParams = null)
    {
        return $this->queryAsyncWithHttpInfo($productCode, $bizScene, $outBizNo, $orderId, $payFundOrderId, $customizedParams)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @deprecated
     */
    public function queryAsyncWithHttpInfo($productCode = null, $bizScene = null, $outBizNo = null, $orderId = null, $payFundOrderId = null, ?CustomizedParams $customizedParams = null)
    {
        $returnType = '\Alipay\OpenAPISDK\Model\AlipayFundTransCommonQueryResponseModel';
        $request = $this->queryRequest($productCode, $bizScene, $outBizNo, $orderId, $payFundOrderId, $customizedParams);
     ...
     */
    public function queryRequest($productCode = null, $bizScene = null, $outBizNo = null, $orderId = null, $payFundOrderId = null, ?CustomizedParams $customizedParams = null)
    {

        $resourcePath = '/v3/alipay/fund/trans/common/query';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        $queryParams = array_merge($queryParams, ObjectSerializer::toQueryValue(
            $payFundOrderId,
            'pay_fund_order_id', // param base name
            'string', // openApiType
            'form', // style
            true, // explode
            false // required
